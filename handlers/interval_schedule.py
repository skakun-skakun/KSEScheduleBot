...
async def week_schedule_ask_email(message: Message, state: FSMContext):
    ...
    await state.set_state(ParseScheduleByInterval.chosen_email_or_name)


@router.message(ParseScheduleByInterval.chosen_email_or_name)
...
async def choosing_start_date(call: CallbackQuery, callback_data: CallbackQuery, state: FSMContext):
    ...
    if selected and date:
        ...
        if not subjects:
            await call.message.answer("No schedule found by given credentials")
            await state.clear()
            return
        ...
        if not formated_schedule:
            await call.message.answer("<b>No schedule found by given credentials and interval</b>")
        else:
            if len(formated_schedule) > 4000:
                ...
                if len(formated_schedule) > 4000:
                    ...
                    await state.clear()
                    return
                await call.message.answer(f"Schedule for <b>{user_data['chosen_email_or_name']}</b>\n\n" + formated_schedule, parse_mode=ParseMode.HTML)
            await call.message.answer(f"Schedule for <b>{user_data['chosen_email_or_name']}</b>\n\n" + formated_schedule, parse_mode=ParseMode.HTML)
        await state.clear()
