...
async def week_schedule_ask_email(message: Message, state: FSMContext):
    await message.answer("Enter email (without @kse.org.ua)\nOR\nSurname and name in Ukrainian (Example: Скакун Остап):")
    await state.set_state(ParseScheduleByWeek.chosen_email_or_name)


@router.message(ParseScheduleByWeek.chosen_email_or_name)
async def choosing_email(message: Message, state: FSMContext):
    await state.update_data(chosen_email_or_name=message.text.lower())
...
async def choosing_week(message: Message, state: FSMContext):
    ...
    if ' ' in user_data['chosen_email_or_name']:
        surname, name = user_data['chosen_email_or_name'].title().split(' ')
        subjects = parser.parse_students_subjects(surname=surname, name=name)
    else:
        email = user_data['chosen_email_or_name']
        if '@kse.org.ua' not in email:
            email += '@kse.org.ua'
        subjects = parser.parse_students_subjects(email=email)

    if not subjects:
        await message.answer("No schedule found by given credentials")
        await state.clear()
        return
    ...
    if not formated_schedule:
        await message.answer("<b>No schedule found by given credentials and week</b>")
    else:
        if len(formated_schedule) > 3500:
            formated_schedule = parser.compact_format_schedule(schedule)
            if len(formated_schedule) > 3500:
                await message.answer(f"<b>Schedule is too long</b>",
                                     parse_mode=ParseMode.HTML)
                await state.clear()
                return
            await message.answer(f"Schedule for <b>{user_data['chosen_email_or_name']}</b> for <b>Week {message.text}</b>\n\n" + formated_schedule,
                                 parse_mode=ParseMode.HTML, reply_markup=adj_week_kb(user_data['chosen_email_or_name'], int(message.text)))
        await message.answer(f"Schedule for <b>{user_data['chosen_email_or_name']}</b> for <b>Week {message.text}</b>\n\n" + formated_schedule,
                             parse_mode=ParseMode.HTML, reply_markup=adj_week_kb(user_data['chosen_email_or_name'], int(message.text)))
    await state.clear()
...
async def inline_week_schedule(call: CallbackQuery, state: FSMContext):
    _, email_or_name, week = call.data.split("`")

    if ' ' in email_or_name:
        surname, name = email_or_name.title().split(' ')
        subjects = parser.parse_students_subjects(surname=surname, name=name)
    else:
    ...
    if not subjects:
        ...
        await call.message.edit_reply_markup(reply_markup=adj_week_kb(email_or_name, int(week)))
        await state.clear()
        return
    ...
    if not formated_schedule:
        await call.message.edit_text("<b>No schedule found by given credentials and week</b>",
                                     parse_mode=ParseMode.HTML)
        await call.message.edit_reply_markup(reply_markup=adj_week_kb(email_or_name, int(week)))
    else:
        if len(formated_schedule) > 3500:
            ...
            if len(formated_schedule) > 3500:
                ...
                await state.clear()
                return
            await call.message.edit_text(f"Schedule for <b>{email_or_name}</b> for <b>Week {week}</b>\n\n" + formated_schedule,
                                         parse_mode=ParseMode.HTML)
            await call.message.edit_reply_markup(reply_markup=adj_week_kb(email_or_name, int(week)))
